...
const StudentFooter = ({ onAttendanceClick, onQuizDungeonClick }) => {
  const { goQuizList, goEvent } = useCustomNavigate();

  // 1. 일일 퀘스트 / 출석체크 버튼 클릭 처리
  const handleAttendanceClick = (e) => {
    if (onAttendanceClick) {
      onAttendanceClick(e); // 상위 컴포넌트에서 전달한 콜백 함수 실행
    }
    goEvent(); // 페이지 이동 처리
  };
  ...
  const handleQuizDungeonClick = (e) => {
    if (onQuizDungeonClick) {
      onQuizDungeonClick(e); // 상위 컴포넌트에서 전달한 콜백 함수 실행
    }
    goQuizList(); // 페이지 이동 처리
  };

  return (
    <footer className="w-full bg-slate-900/90 border-t border-slate-800/80 px-8 py-4 flex items-center justify-center gap-6 relative z-10 shrink-0 font-sans">
      <button
        type="button"

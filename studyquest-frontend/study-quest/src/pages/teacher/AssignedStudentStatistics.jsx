...
export default function AssignedStudentStatistics() {
  ...
  const fetchStudentLogs = async (isInit = false) => {
    ...
    try {
      ...
      const response = await jwtAxios.get(
        `http://localhost:8080/teachers/student-results?teacherNo=${teacherNo}`
      );
  ...
  return (
    <div className="space-y-8">
      {/* [학생 관리] 스타일과 통일된 5px 초슬림 다크 스크롤바 */}
      <style>{`
        ::-webkit-scrollbar {
          width: 5px !important;
          ...
          {!loading &&
            ...
            studentLogs.map((log, idx) => (
              ...
              >
                ...
                <span className="text-gray-300 flex-1 text-center font-medium">
                  {log.quizTitle || "제목 없음"}
                </span>
                <span className="text-gray-500 text-xs w-28 text-center">
                  {log.resultDate || "-"}
                </span>
